function updateLiveStats() {
  ...
  document.getElementById('visitCount').innerText = visits.length;
  ...
  months.forEach((m, i) => {
    ...
    row.innerHTML = `
      ...
      <td>15k</td>
      <td>${i === 0 ? '14.2k' : '-'}</td>
      <td class="thick-border">${i === 0 ? '790' : '-'}</td>
      <td style="color:#3b82f6;">${i === 0 ? '45' : '-'}</td>
      <td style="color:#22c55e;">${i === 0 ? '12' : '-'}</td>
    `;
  });

  const achievedCtx = document.getElementById('achievedChart').getContext('2d');
  new Chart(achievedCtx, {
    type: 'doughnut',
    data: {
      datasets: [{
        data: [82, 18],
